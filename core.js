...
function getFinalSpecial(stat){
  let base = state.special[stat] || 5;
  const race = getRace();
  const subset = getSubset(race);

  if(race && race.specialMods && race.specialMods[stat]){
    base += race.specialMods[stat];
    
}
  if(subset && subset.specialMods && subset.specialMods[stat]){
    base += subset.specialMods[stat];
  }
...
function getFinalDR(){
  ...
  if(race && race.drBase) dr += race.drBase;
  if(subset && subset.drMod) dr += subset.drMod;

  return dr;
}

function renderSpecialRow(el){
  if(!el) return;

  const stats = ["STR","PER","END","CHA","INT","AGI","LCK"];

  el.innerHTML = stats.map(s=>{
    const val = getFinalSpecial(s);
    const mod = scoreToMod(val);
    const sign = mod>=0?"+":"";
    return `
      <div class="special-box">
        <div class="special-label">${s}</div>
        <div class="nixie-medium">${val}</div>
        <div class="special-mod">(${sign}${mod})</div>
      </div>
    `;
  }).join("");
}
...
function getSkillMod(skillId){
  ...
  // Subset flat skill mods
  const race = getRace();
  const subset = getSubset(race);
  if(subset && subset.skillMods && subset.skillMods[skillId]){
    mod += Number(subset.skillMods[skillId]) || 0;
  }
  // SPECIAL modifier
  mod += getModifier(skill.stat);
  ...
  // === PERK BONUSES (CANONICAL) ===
  PERKS.forEach(perk=>{

    const rank = getPerkOwned(perk.id);
    if(!rank || rank <= 0) return;
    if(!perk.ranks) return;

    const text = perk.ranks[rank-1] || "";

    // Match "+2 Guns" style
    const match = text.match(/\+(\d+)\s+([A-Za-z_\/]+)/i);
    if(!match) return;

    const val = Number(match[1]);
    const token = (match[2]||"").toLowerCase();
    const skills = token.split("/").map(s=>s.trim()).filter(Boolean);
...
function renderPerkTile(perk, overspent, isPickLevel, remaining){
  ...
  const owned = state.perks[perk.id] || 0;
  rank.textContent = `Rank: ${owned}/${perk.ranks}`;

  const pick = document.createElement("button");
  pick.textContent = "Pick";
  pick.disabled = !isPickLevel || remaining<=0 || owned>=perk.ranks || overspent;

  pick.onclick = ()=>buyPerk(perk.id);

  const unpick = document.createElement("button");
  unpick.textContent = "Unpick";
  unpick.disabled = owned<=0;

  unpick.onclick = ()=>unbuyPerk(perk.id);

  card.append(title, rank, desc, pick, unpick);
  return card;
}
...
function renderPerks() {
  ...
  container.innerHTML = `
    ...
    <div class="perk-grid">
      ...
      <div class="perk-family" id="perk-stealth-mobility">
        <h3>Stealth / Mobility</h3>
        <div class="perk-family-body" id="perk-stealth-mobility-body"></div>
      </div>

      
      <div class="perk-family" id="perk-survival">
        <h3>Survival & Medicine</h3>
        <div class="perk-family-body" id="perk-survival-body"></div>
      </div>

      
      <div class="perk-family" id="perk-crafting-tech">
        <h3>Crafting & Technical</h3>
        <div class="perk-family-body" id="perk-crafting-tech-body"></div>
      </div>
    </div>
  `;
  ...
  const craftBody = document.getElementById("perk-crafting-tech-body");
  if(craftBody){

    const craftPerks = PERKS.filter(p => String(p.category||"").toLowerCase() === "crafting_technical");

    if(craftPerks.length === 0){
      craftBody.classList.add("empty");
      craftBody.textContent = "No crafting/technical perks loaded";
    } else {

      craftPerks.forEach(perk => {
        const pr = perkNextRank(perk);
        const gate = canPickPerkNow(perk);
        const req = checkPerkReq(perk);
        ...
        const desc = document.createElement("div");
        desc.className = "perk-desc";
        const nextText = (perk.ranks && perk.ranks[pr.nextIndex]) 
          ? perk.ranks[pr.nextIndex] 
          : (perk.ranks && perk.ranks[0] ? perk.ranks[0] : "");
        desc.textContent = nextText;
        ...
        tile.append(title, meta, desc);

        const rankList = document.createElement("div");
        rankList.className = "perk-ranks";

        if(Array.isArray(perk.ranks) && perk.ranks.length > 1){
          rankList.innerHTML = perk.ranks.map((t,i)=>{
            const ownedMark = (i < pr.owned) ? " owned" : "";
            const nextMark = (i === pr.nextIndex && pr.hasNext) ? " next" : "";
            return `<div class="perk-rank${ownedMark}${nextMark}">Rank ${i+1}: ${t}</div>`;
        ...
        craftBody.appendChild(tile);
      });
    }
  }

const survBody = document.getElementById("perk-survival-body");
  if(survBody){

    const survPerks = PERKS.filter(p => String(p.category||"").toLowerCase() === "survival_medicine");

    if(survPerks.length === 0){
      survBody.classList.add("empty");
      survBody.textContent = "No survival/medicine perks loaded";
    } else {

      survPerks.forEach(perk => {
        const pr = perkNextRank(perk);
        const gate = canPickPerkNow(perk);
        const req = checkPerkReq(perk);
        ...
        survBody.appendChild(tile);
      });
    }
  }

const smBody = document.getElementById("perk-stealth-mobility-body");
  if(smBody){

    const smPerks = PERKS.filter(p => String(p.category||"").toLowerCase() === "stealth_mobility");

    if(smPerks.length === 0){
      smBody.classList.add("empty");
      smBody.textContent = "No stealth/mobility perks loaded";
      return;
    }
...
// Unified perk handler



// Unified perk handler (stable)



// Unified perk handler (gated + toggle)


// Unified perk handler (sequential ranks)
...
// Clean unified perk handler



// Unified perk handler (single definition)



// Unified perk handler (combat-only baseline)
function buyPerk(perkId){

  if(typeof isPerk === "function" && isPerk(perkId)){
    if(typeof unpickPerk === "function"){
      unpickPerk(perkId);
    }
  }
  else if(typeof canPickPerk === "function" && canPickPerk(perkId)){
    if(typeof pickPerk === "function"){
      pickPerk(perkId);
    }
  }
  ...
  else if(typeof renderPerk === "function"){
    renderPerk();
  }
}
...
function calcHPMax(){
  const base = (RACES[state.raceId]?.hpBase || 10);
  const end = state.special.END || 5;
  const lvl = Number(state.level)||1;
  return base + (2*end) + lvl;
}
...
export function modHP(n){
  const max = calcHPMax();
  if(state.hpCurrent === null) state.hpCurrent = max;

  state.hpCurrent = clamp(state.hpCurrent + n, 0, max);
  renderSheet();
}

export function modRAD(n){
  state.rad = clamp((state.rad||0)+n,0,1000);
  renderSheet();
}
...
function calcSupportStats(){
  const agi = state.special.AGI || 5;
  const str = state.special.STR || 5;

  const ap = 5 + Math.floor((agi - 5) / 2);
  const init = Math.max(0, Math.floor((agi - 5) * 2));
  const move = Math.max(1, Math.floor(agi / 2));

  const race = getRace();
  const subset = getSubset(race);
  const dr = race ? computeDrBase(race, subset) : 0;

  const dt = state.armorDT || 0;
  const carry = (str * 10);

  return { ap, dr, dt, carry, init, move };
}

function renderSupportStats(el){
  if(!el) return;

  const s = calcSupportStats();

  el.innerHTML = `
    <div class="combat-grid">
      <div class="nixie-medium">AP ${s.ap}</div>
      <div class="nixie-medium">DR ${s.dr}</div>
      <div class="nixie-medium">DT ${getEquippedDTBonus()}</div>
      <div class="nixie-medium">CAR ${s.carry}</div>
      <div class="nixie-medium">INIT ${s.init}</div>
      <div class="nixie-medium">MOV ${s.move}</div>
    </div>
  `;
}

export function renderSheet(){
// skills will render later
  ...
  const radEl = document.getElementById("radDisplay");
  const supEl = document.getElementById("supportStats");

  if(!nameEl) return;

  const race = RACES[state.raceId]?.name || "Unknown";
  const subset = RACES[state.raceId]?.subsets?.[state.subsetId]?.name || "";
  const lvl = state.level||1;

  const hpMax = calcHPMax();
  if(state.hpCurrent===null) state.hpCurrent = hpMax;

  nameEl.textContent = (state.name||"UNNAMED").toUpperCase();
  metaEl.textContent = `${race} — ${subset} | Level ${lvl}`;

  hpEl.textContent = pad(state.hpCurrent,3)+"/"+pad(hpMax,3);
  radEl.textContent = pad(state.rad||0,4);

  const ap = 5 + Math.floor(((state.special.AGI||5)-5)/2);
  const dr = RACES[state.raceId]?.drBase || 0;
  const carry = (state.special.STR||5)*10;
  const init = (state.special.AGI||5)-5;

  renderSupportStats(supEl);
  const specEl = document.getElementById('specialRow');
  renderSpecialRow(specEl);
  renderSheetSkills();
}
...
// expose tracker controls for inline onclick buttons
window.modHP = modHP;
window.modRAD = modRAD;





function getSheetSkillTotals(){
  const out = {};
  if(typeof SKILLS === "undefined") return out;

  SKILLS.forEach(s=>{
    try{
      out[s.id] = Number(getSkillMod(s.id)) || 0;
    }catch(e){
      out[s.id] = 0;
    }
  });
...
function renderSheetSkills(){
  ...
  let el = document.getElementById("sheetSkillsWrap");

  if(!el){
    const spec = document.getElementById("specialRow");
    if(!spec || !spec.parentElement) return;

    el = document.createElement("div");
    el.id = "sheetSkillsWrap";
    el.className = "sheet-skills-equipped";

    spec.parentElement.appendChild(el);
  }

  const skills = getSheetSkillTotals();
  ...
  Object.entries(skills).forEach(([id,val])=>{
    ...
    const name = (typeof SKILLS !== "undefined"
  ...
  const equippedHtml = `
    <div class="sheet-section equipped-panel">
      ${renderEquipped()}
    </div>
  `;

  el.innerHTML = skillsHtml + equippedHtml;
}
...
function renderEquipped(){
  ...
  return `
    ...
    <div class="equipped-list">
      <div>Head: ${e.head || "—"}</div>
      <div>Torso: ${e.torso || "—"}</div>
      <div>Arms: ${e.arms || "—"}</div>
      <div>Legs: ${e.legs || "—"}</div>
      <div>Primary: ${e.primary || "—"}</div>
      <div>Secondary: ${e.secondary || "—"}</div>
      <div>Utility: ${e.utility || "—"}</div>
    </div>
  `;
}


/* === Inventory Phase 2 (Clean Rebuild) === */

function addInventoryItem(name, slot){
  if(!name) return;
...
function removeInventoryItem(id){
  ...
  if(state.equipped){
    Object.keys(state.equipped).forEach(k=>{
      if(state.equipped[k]===id) state.equipped[k]=null;
    });
  }
...
function unequipInventorySlot(slot){
  if(state.equipped){
...
function renderInventory(){
  ...
  inv.forEach(i=>{
    html += `
      <div class="inv-row">
        <span>${i.name} (${i.slot})</span>
        <div>
          <button onclick="equipInventoryItem(${i.id})">Equip</button>
          <button onclick="removeInventoryItem(${i.id})">✕</button>
        </div>
      </div>
    `;
  });
  ...
  html += `
      ...
      <div class="inv-right">
        ...
        <button onclick="addInventoryItem(
          ...
          document.getElementById('invSlot').value
        )">Add</button>

        <h3>Equipped</h3>
        <div class="inv-eq">
          ${Object.entries(state.equipped||{}).map(([k,v])=>{
            const it = inv.find(i=>i.id===v);
            return `<div>${k}: ${it?it.name:"—"}
              <button onclick="unequipInventorySlot('${k}')">Unequip</button>
            </div>`;
          }).join("")}
        </div>
      </div>
...
/* === Phase 3A: Starter Test Armor === */
function seedStarterArmor(){
  if(!state.inventory) state.inventory = [];
  if(state.inventory.length > 0) return;

  state.inventory = [
    { id: 1, name: "Leather Helmet", slot: "head", dt: 1, dr: 0 },
    { id: 2, name: "Leather Chest", slot: "torso", dt: 3, dr: 0 },
    { id: 3, name: "Leather Arms", slot: "arms", dt: 2, dr: 0 },
    { id: 4, name: "Leather Legs", slot: "legs", dt: 2, dr: 0 }
  ];
}
